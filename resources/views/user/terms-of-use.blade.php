<x-main-layout>
    <x-slot name="meta">
        <title>{{ $terms->meta_title }} - {{$general_setting->site_name}}</title>
        <meta name="keyword" content="{{ $terms->meta_keyword }}">
...
<meta name="twitter:image" content="{{ Storage::disk('images')->url($meta_data->meta_image) ?? '' }}" />
    ...
    <section class="relative overflow-hidden z-10 pt-12.5 min-h-screen bg-cover bg-center"
        style="background-image: url({{ Storage::disk('images')->url($terms->background_image) }});">
        <!-- Gradient Overlay -->
